...
public class SrsPublisher {
    ...
    public Bitmap  bgd = null;

    public SrsPublisher(final SrsCameraView view) {
        mCameraView = view;
        mCameraView.setPreviewCallback(new SrsCameraView.PreviewCallback() {
            @Override
            public void onGetRgbaFrame(byte[] data, int width, int height,Camera camera) {
                calcSamplingFps();
                if(isSeg){
                    Bitmap bitmap = ImageUtils.rgba2Bitmap(data,width,height);

                    Bitmap transbp = ImageUtils.resizeBitmap(bitmap,128,128);
                    Bitmap bgd = Bitmap.createScaledBitmap(
                            mCameraView.getBitmap(),width,height , false);
                    segmentor.segmentFrame(transbp,1,bitmap,bgd,width,height);

                    byte[] data_result =ImageUtils.bitmap2RGBA(segmentor.result,width,height); //Get the bytes array of the bitmap
//                    测试用
//                    if(f)
//                    {
////                        ImageUtils.saveBitmap(ImageUtils.rgba2Bitmap(data_result,width,height),"/atest.jpg");
//                        ImageUtils.saveBitmap(segmentor.result,"/aresult.jpg");
////                        ImageUtils.saveBitmap(bitmap,"/bitmap.jpg");
//                        ImageUtils.saveBitmap(bgd,"/bgd.jpg");
////                    Log.i("segmentorr","datalen:"+data.length+"data_result:"+data_result.length);
//                    f = false;
//                    }
                    data = data_result;
//                    camera.addCallbackBuffer(data_result);
                   mCameraView.setImageBitmap(segmentor.result);
                }
                else {
                    Log.i("segmentorr","seg is null");
                }
    ...
    public void setSegmentor(ImageSegmentor segmentor) {
        this.segmentor = segmentor;
        isSeg = true;
    }
    public void removeSegmentor(){
        isSeg = false;
    }
}
